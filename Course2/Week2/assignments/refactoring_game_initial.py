'''
Данный файл содержит исходное задание.

Вам предлагается программа — заставка, в которой по экрану «летает» кривая.
При нажатии на экране мышкой — к кривой добавляется новая «опорная» точка.
Сглаживание ломаной из опорный точек происходит при помощи специально написанного алгоритма.

В этой работе, Вам предстоит провести рефакторинг предложенной программы, переместив максимум функционала в соответствующие классы.
В процессе рефакторинга, Вам придётся создать класс двумерных векторов, который можно будет использовать и в других Ваших программах.
Несмотря на то, что некоторые (требуемые по заданию) функции не используются (длина векторов, деление вектора на число, скалярное умножение и пр.)
или же могут быть использованы только один раз (получение пары целых чисел для рисования кривых), их реализация необходима для получения более полезного результата. В то же время, для решения дополнительных задач Вам может потребоваться пополнить классы методами, не указанными в задании.
'''

'''
Вам необходимо провести рефакторинг кода:

1. Реализовать класс 2-мерных векторов Vec2d — определить основные математические операции:
...
2. Реализовать класс замкнутых ломаных Polyline, с возможностями:
- добавление в ломаную точки (Vec2d) c её скоростью
- пересчёт координат точек (set_points)
- отрисовка ломаной (draw_points)

3. Реализовать класс Knot — потомок класса Polyline — в котором добавление и пересчёт координат
инициируют вызов функции get_knot для расчёта точек кривой по добавляемым опорным.
...
if __name__ == "__main__":
    ...
    while working:
        ...
        '''
        points - list of tuples with each point cords inside each tuple
        '''
        draw_points(points)
